...
exports.updateNote = AsyncHandler(async (req, res, next) => {
    ...
    if (note.user.toString() !== loggedInUser.id.toString()) {
        return next(new ErrorResponse(`${loggedInUser.name} not authorized to update this note`, 401))
    }
...
exports.deleteNote = AsyncHandler(async (req, res, next) => {
    ...
    // if user is not owner of the note simply return error
    if (note.user.toString() !== loggedInUser.id.toString()) {
        return next(new ErrorResponse(`${loggedInUser.name} not authorized to delete this note`, 401))
    }
