...
// desc : update logged in user profile
// route : PUT /api/v1/user/profile/picture
// access : private
exports.updateProfilePicture = AsyncHandler(async (req, res, next) => {
  ...
  profilePicture.name = `photo_${loggedInUser}${
    path.parse(profilePicture.name).ext
  }`;
  // move file to uploads folder
  profilePicture.mv(
    `${process.env.PROFILE_PICTURE_UPLOAD_PATH}/${profilePicture.name}`,
    async (err) => {
      if (err) {
        console.log(err);
      ...
      const profilePicturePath = `${backendUrl}/uploads/profile/${profilePicture.name}`;
...
exports.deleteProfilePicture = AsyncHandler(async (req, res, next) => {
  ...
  if (user.profilePicture === process.env.DEFAULT_PROFILE_PICTURE) {
    return next(
      new ErrorResponse('You have not uploaded a profile picture', 400)
    );
  }
