...
exports.register = AsyncHandler(async (req, res, next) => {
  ...
  if (!confirmEmailToken) {
    return next(
      new ErrorResponse(
        'Error occured while generating email confirmation token',
        500
      )
    );
  }
  ...
  setTimeout(async () => {
    try {
      ...
      if (user.isEmailConfirmed === false) {
        ...
        console.log(
          'token expired, registration/signup request deleted!',
          user.email
        );
...
exports.forgotPassword = AsyncHandler(async (req, res, next) => {
  ...
  if (!user || user.isEmailConfirmed === false) {
    return next(
      new ErrorResponse(
        `no user found with ${req.body.email}, try right one!`,
        404
      )
    );
  }
  ...
  res.status(200).json({
    success: true,
    message:
      'forget password token sent successfully!, please check your email',
    data: {},
  });
});
